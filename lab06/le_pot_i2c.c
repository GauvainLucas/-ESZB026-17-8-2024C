...
#define BUFFER_SIZE 7      // 5 posicoes do buffer do Arduino: de 0x00 a 0x06
...
int main(int argc, char **argv){
   ...
   if(argc==2){
     ...
     char alertbuf[] = {0x02, 0, 0};    // limite do alarme com 'comando' 0x10
   ...
   if(write(file,&command,1)!=1){
	perror("Falha ao enviar o comando 0x10.\n");
	return 1;
   }
   ...
   valorLED = rec[4];
   int inteiro_volt, decimal_volt;
   inteiro_volt=rec[5];
   decimal_volt=rec[6];
   printf("O valor lido foi %d.\n", valor);
   printf("O valor lido em volts foi %d.02%d V\n",inteiro_volt,decimal_volt);
